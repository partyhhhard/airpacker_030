...
/* USER CODE BEGIN PFP */
void motorControlTaskFunc( void const *argument );
void temperatureControlTaskFunc( void const *argument );
/* USER CODE END PFP */

/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t heap_ram[0x1100] @ "RAM_region";
...
int main(void)
{
  ...
  DBGMCU->APB1FZ = __HAL_DBGMCU_FREEZE_TIM1();
  /* USER CODE END 1 */
  ...
  /* add threads, ... */
 osThreadDef( motorControl, motorControlTaskFunc, osPriorityNormal, 0, 128 );
 motorControlTaskHandle = osThreadCreate( osThread( motorControl ), NULL );
 
 osThreadDef( temperatureControl, temperatureControlTaskFunc, osPriorityNormal, 0, 128 );
